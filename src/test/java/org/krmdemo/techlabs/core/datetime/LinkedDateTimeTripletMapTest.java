...
public class LinkedDateTimeTripletMapTest {
    ...
    void testEventsForTwoDays() {
        ...
        System.out.println("---- original schedules of events for two days: ----");
        System.out.println(buildSchedule(eventsMapForTwoDays));
        System.out.println("---- schedules of events for two days with forward clean: ----");
        System.out.println(buildScheduleForwardClean(eventsMapForTwoDays));
        System.out.println("---- schedules of events for two days with backward clean: ----");
        System.out.println(buildScheduleBackwardClean(eventsMapForTwoDays));

        assertThat(buildSchedule(eventsMapForTwoDays)).isEqualTo("""
            |  1) 2025-10-15 (Wed) 07:40 | wake up today!
            |  2) 2025-10-15 (Wed) 08:00 | breakfast
            |  3) 2025-10-15 (Wed) 12:25 | lunch
            |  4) 2025-10-15 (Wed) 18:45 | happy hours
            |  5) 2025-10-16 (Thu) 00:00 | midnight
            |  6) 2025-10-16 (Thu) 08:53 | wake up tomorrow !!!""");
        assertThat(buildScheduleForwardClean(eventsMapForTwoDays)).isEqualTo("""
            |  1) 2025-10-15 (Wed) 07:40 | wake up today!
            |  2)                  08:00 | breakfast
            |  3)                  12:25 | lunch
            |  4)                  18:45 | happy hours
            |  5)        -16 (Thu) 00:00 | midnight
            |  6)                  08:53 | wake up tomorrow !!!""");
        assertThat(buildScheduleBackwardClean(eventsMapForTwoDays)).isEqualTo("""
            |  6) 2025-10-16 (Thu) 08:53 | wake up tomorrow !!!
            |  5)                  00:00 | midnight
            |  4)        -15 (Wed) 18:45 | happy hours
            |  3)                  12:25 | lunch
            |  2)                  08:00 | breakfast
            |  1)                  07:40 | wake up today!""");
    }
    ...
    void testEventsForVacation() {
        ...
        System.out.println("---- schedules of events for two days with forward clean: ----");
        System.out.println(buildScheduleForwardClean(eventsMapForVacation));
        System.out.println("---- schedules of events for two days with backward clean: ----");
        System.out.println(buildScheduleBackwardClean(eventsMapForVacation));
        ...
        assertThat(buildScheduleForwardClean(eventsMapForVacation)).isEqualTo("""
            |  1) 2024-10-15 (Tue) 17:40 | book a hotel
        ...
        assertThat(buildScheduleBackwardClean(eventsMapForVacation)).isEqualTo("""
            | 10) 2025-01-08 (Wed) 09:00 | vacation end
            |  9)        -07 (Tue) 11:45 | take off home
    ...
    private String buildScheduleForwardClean(SequencedMap<Integer, Event> eventsMap) {
        ...
        return eventsMap.keySet().stream().map(eventNum -> {
            ...
            return String.format("|%3d) %7s%10s%s | %s", eventNum,
                linkedTriplet.isYearAnMonthTheSameAsPrev() ? "" : linkedTriplet.getYearAndMonth(),
                linkedTriplet.isDayOfMonthAndWeekTheSameAsPrev() ? "" : linkedTriplet.getDayOfMonthAndWeek(),
                linkedTriplet.isHoursMinutesTheSameAsPrev() ? "" : linkedTriplet.getHoursMinutes(),
                event.subject
            );
        }).collect(Collectors.joining(System.lineSeparator()));
    }

    private String buildScheduleBackwardClean(SequencedMap<Integer, Event> eventsMap) {
        SequencedMap<Integer, LinkedTriplet> linkedTripletsMap =
            LinkedDateTimeTripletMap.createByEpochSecondsInt(eventsMap, Event::epochSeconds);
        assertThat(linkedTripletsMap).hasSize(eventsMap.size());
        assertThat(linkedTripletsMap.keySet()).isEqualTo(eventsMap.keySet());
        return eventsMap.reversed().keySet().stream().map(eventNum -> {
            ...
            LinkedTriplet linkedTriplet = linkedTripletsMap.get(eventNum);
            return String.format("|%3d) %7s%10s%s | %s", eventNum,
                linkedTriplet.isYearAnMonthTheSameAsNext() ? "" : linkedTriplet.getYearAndMonth(),
                linkedTriplet.isDayOfMonthAndWeekTheSameAsNext() ? "" : linkedTriplet.getDayOfMonthAndWeek(),
                linkedTriplet.isHoursMinutesTheSameAsNext() ? "" : linkedTriplet.getHoursMinutes(),
                event.subject
            );
        }).collect(Collectors.joining(System.lineSeparator()));
    }
